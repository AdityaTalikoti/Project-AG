...
//  SIGN IN VIEW
// ═══════════════════════════════════
function SignInView({ onSwitch, onForgot }) {
  ...
  const handleSubmit = (e) => {
    ...
    setLoading(true);
    setBanner('');
    // Simulate API — always show generic error for demo
    setTimeout(() => { setLoading(false); setBanner('Invalid credentials. Please try again.'); }, 1500);
  };
      ...
      <div className="auth-form-panel">
        <div className="auth-card">
          ...
          {view !== 'forgot' && (
            <>
              ...
              <div className="auth-tabs" role="tablist">
                <button role="tab" aria-selected={view==='signin'} className={`auth-tab${view==='signin'?' active':''}`} onClick={()=>setView('signin')} id="tab-signin">Sign In</button>
          ...
          <div aria-live="polite">
            {view === 'signin' && <SignInView onSwitch={()=>setView('signup')} onForgot={()=>setView('forgot')}/>}
            {view === 'signup' && <SignUpView onSwitch={()=>setView('signin')}/>}
